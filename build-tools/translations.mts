...
export class ParsedUIText {
	...
	constructor(source: string, filename = '<ui>') {
		...
		for (let i = 0; i < this.lines.length; i++) {
			...
			const entry = {
				key, region, start: i, end, rawLines: this.lines.slice(i, end + 1),
	...
	update(calls: TLCalls): { source: string, added: string[], errors: string[] } {
		...
		for (const [key, call] of calls) {
			...
			const region = call.regions.size === 1 ? [...call.regions][0] : 'Generic UI';
		...
		this.updateNotUsedMarkers(lines, calls);
		const added: string[] = [];
	...
	private updateNotUsedMarkers(lines: string[], calls: TLCalls): void {
		for (const entry of this.entries) {
	...
	sync(locale: ParsedUIText): { source: string, comparison: UITextComparison } {
		...
		for (const entry of this.entries) {
			...
			const localized = locale.entriesByKey.get(entry.key);
			if (localized) {
				output.push(...localized.localComments, ...localized.rawLines);
			} else {
...
export function updateUIText(options: { sync?: boolean } = {}): void {
	...
	for (const file of localeFiles) {
		...
		const synced = template.sync(locale);
