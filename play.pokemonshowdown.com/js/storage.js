...
Storage.exportAllTeams = function () {
	...
	for (var i = 0, len = Storage.teams.length; i < len; i++) {
		...
		buf += Storage.exportTeam(team.team, team.gen, false, team.format);
		buf += '\n';
	}
	return buf;
};
...
Storage.exportTeam = function (team, gen, hidestats, format) {
	if (!team) return "";
	...
	for (var i = 0; i < team.length; i++) {
		...
		if (gen === 9 && format && !format.includes('champions')) {
...
Storage.nwSaveTeam = function (team) {
	...
	fs.writeFile(this.dir + 'Teams/' + filename, Storage.exportTeam(team.team, team.gen, false, team.format).replace(/\n/g, '\r\n'), function () {});
};
