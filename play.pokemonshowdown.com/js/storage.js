		...
		if (!hues) {
			switch (bgid) {
				...
				case "charizards":
					...
					attrib =
						'<a href="https://lit.link/en/seiryuuden" target="_blank" class="subtle">"Charizards" <small>background by Jessica Valencia</small></a>';
					break;
				case "psday":
					hues = [
...
Storage.compareTeams = function (serverTeam, localTeam) {
	...
	if (mons.length !== otherMons.length || matches !== otherMons.length)
		return "rename";
	...
	try {
		...
		Storage.whenAppLoaded(function (app) {
			app.addPopup(Popup, {
				type: "modal",
				htmlMessage:
					'Your teams are corrupt and could not be loaded. :( Staff may be able to recover it from your data. This is your data; you should save it somewhere:<br /><textarea rows="10" cols="60">' +
...
Storage.exportTeam = function (team, gen, hidestats) {
	...
	for (var i = 0; i < team.length; i++) {
		...
		if (gen === 9) {
			...
			text +=
				"Tera Type: " +
				(curSet.teraType || species.requiredTeraType || species.types[0]) +
				"  \n";
		}
		if (!hidestats) {
			var first = true;
