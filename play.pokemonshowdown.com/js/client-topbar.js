(function ($) {
	this.Topbar = Backbone.View.extend({
		...
		initialize: function () {
			...
			this.$el.html(
				'<img class="logo" src="' +
					Dex.resourcePrefix +
					'pokemonshowdownbeta.png" srcset="' +
					Dex.resourcePrefix +
					'pokemonshowdownbeta@2x.png 2x" alt="Pok&eacute;mon Showdown! (beta)" width="146" height="44" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>',
			);
		...
		updateUserbar: function () {
			...
			var color = away
				? "color:#888;"
				: BattleLog.hashColor(app.user.get("userid"));
			if (!app.user.loaded) {
				...
				buf =
					...
					(away ? ' data-away="true"' : "") +
					(status
						? 'data-status="' + BattleLog.escapeHTML(status) + '"'
						: "") +
					' style="' +
					color +
					'"><i class="fa fa-user" style="color:' +
					(away ? "#888;" : "#779EC5") +
					'"></i> <span class="usernametext">' +
					BattleLog.escapeHTML(name) +
					"</span></span>";
			} else {
			...
			buf +=
				' <button class="icon button" name="openSounds" title="Sound" aria-label="Sound"><i class="' +
				(Dex.prefs("mute") ? "fa fa-volume-off" : "fa fa-volume-up") +
				'"></i></button> <button class="icon button" name="openOptions" title="Options" aria-label="Options"><i class="fa fa-cog"></i></button>';
		...
		toggleMute: function () {
			...
			app.topbar
				...
				.html(
					'<i class="' +
						(muted ? "fa fa-volume-off" : "fa fa-volume-up") +
						'"></i>',
				);
		},
		...
		renderRoomTab: function (room, id) {
			...
			if (!id) id = room.id;
			var buf =
				'<li><a class="roomtab button' +
				(app.curRoom === room || app.curSideRoom === room ? " cur" : "") +
				((room && room.notificationClass) || "") +
				(id === "" || id === "rooms" ? "" : " closable") +
				'" href="' +
				app.root +
				id +
				'"';
			if (room && room.notifications) {
				...
				for (var tag in room.notifications) {
					if (room.notifications[tag].title)
						title += room.notifications[tag].title + "\n";
					if (room.notifications[tag].body)
						title += room.notifications[tag].body + "\n";
				}
			...
			switch (room ? room.type : id) {
				...
				case "mainmenu":
					return (
						buf + '><i class="fa fa-home"></i> <span>Home</span></a></li>'
					);
				case "teambuilder":
					return (
						buf +
						'><i class="fa fa-pencil-square-o"></i> <span>Teambuilder</span></a><button class="closebutton" name="closeRoom" value="' +
						'teambuilder" aria-label="Close"><i class="fa fa-times-circle"></i></button></li>'
					);
				case "ladder":
					return (
						buf +
						'><i class="fa fa-list-ol"></i> <span>Ladder</span></a><button class="closebutton" name="closeRoom" value="' +
						'ladder" aria-label="Close"><i class="fa fa-times-circle"></i></button></li>'
					);
				case "resources":
					return (
						buf +
						'><i class="fa fa-question-circle"></i> <span>Resources</span></a><button class="closebutton" name="closeRoom" value="' +
						'resources" aria-label="Close"><i class="fa fa-times-circle"></i></button></li>'
					);
				case "battles":
					return (
						buf +
						'><i class="fa fa-caret-square-o-right"></i> <span>Battles</span></a><button class="closebutton" name="closeRoom" value="' +
						'battles" aria-label="Close"><i class="fa fa-times-circle"></i></button></li>'
					);
				case "rooms":
					return (
						buf +
						' aria-label="Join chatroom"><i class="fa fa-plus" style="margin:7px auto -6px auto"></i> <span>&nbsp;</span></a></li>'
					);
				case "battle":
					...
					if (!name) {
						var p1 =
							(room.battle && room.battle.p1 && room.battle.p1.name) ||
							"";
						var p2 =
							(room.battle && room.battle.p2 && room.battle.p2.name) ||
							"";
						if (p1 && p2) {
							name =
								"" +
					...
					return (
						buf +
						' draggable="true"><i class="text">' +
						BattleLog.escapeFormat(formatid) +
						"</i><span>" +
						name +
						'</span></a><button class="closebutton" name="closeRoom" value="' +
						id +
						'" aria-label="Close"><i class="fa fa-times-circle"></i></a></li>'
					);
				case "chat":
					return (
						buf +
						' draggable="true"><i class="fa fa-comment-o"></i> <span>' +
						(BattleLog.escapeHTML(room.title) ||
							(id === "lobby" ? "Lobby" : id)) +
						'</span></a><button class="closebutton" name="closeRoom" value="' +
						id +
						'" aria-label="Close"><i class="fa fa-times-circle"></i></a></li>'
				...
				default:
						...
						if (closeBracketIndex > 0) {
							return (
								...
								BattleLog.escapeHTML(
									room.title.slice(1, closeBracketIndex),
								) +
								"</i><span>" +
								BattleLog.escapeHTML(
									room.title.slice(closeBracketIndex + 1),
								) +
								'</span></a><button class="closebutton" name="closeRoom" value="' +
								id +
								'" aria-label="Close"><i class="fa fa-times-circle"></i></a></li>'
					...
					return (
						buf +
						' draggable="true"><i class="fa fa-file-text-o"></i> <span>' +
						(BattleLog.escapeHTML(room.title) || id) +
						'</span></a><button class="closebutton" name="closeRoom" value="' +
						id +
						'" aria-label="Close"><i class="fa fa-times-circle"></i></a></li>'
					);
		...
		updateTabbar: function () {
			...
			if (window.nodewebkit) {
				if (nwWindow.setBadgeLabel)
					nwWindow.setBadgeLabel(
				...
				if (!!$favicon.data("on") !== !!notificationCount) {
					if (notificationCount) {
						$favicon.attr(
							"href",
							Dex.resourcePrefix + "/favicon-notify.ico",
						);
						$favicon.data("on", "1");
					} else {
			...
			if (sideBuf) {
				if (app.curSideRoom) {
					...
					buf +=
						'<ul class="siderooms" style="float:none;margin-left:' +
						margin +
						'px">' +
						sideBuf +
						"</ul>";
			...
			if (offset.top >= 37 || overflow > 0) {
				this.$tabbar.append(
					'<div class="overflow" aria-hidden="true"><button name="tablist" class="button" aria-label="More"><i class="fa fa-caret-down"></i></button></div>',
				);
			}
		...
		updateTabbarMini: function () {
			...
			for (var i in app.rooms) {
				if (
					app.rooms[i] !== app.curRoom &&
					app.rooms[i].notificationClass === " notifying"
				)
					notificationClass = " notifying";
			}
			var buf =
				'<ul><li><a class="button minilogo' +
				notificationClass +
				'" href="' +
				app.root +
				'"><img src="' +
				Dex.resourcePrefix +
				'favicon-256.png" width="32" height="32" alt="Pok&eacute;mon Showdown! (beta)" /><i class="fa fa-caret-down" style="display:inline-block"></i></a></li></ul>';
		...
		dragStartRoom: function (e) {
			...
			app.draggingRoomList = app.roomList
				.map(this.roomidOf)
				.concat("|")
				.concat(app.sideRoomList.map(this.roomidOf));
	...
	var SoundsPopup = (this.SoundsPopup = Popup.extend({
		initialize: function (data) {
			...
			var muted = !!Dex.prefs("mute");
			buf +=
				'<p class="effect-volume"><label class="optlabel">Effect volume:</label>' +
				(muted
					? "<em>(muted)</em>"
					: '<input type="range" min="0" max="100" step="1" name="effectvolume" value="' +
						this.getEffectVolume() +
						'" />') +
				"</p>";
			buf +=
				'<p class="music-volume"><label class="optlabel">Music volume:</label>' +
				(muted
					? "<em>(muted)</em>"
					: '<input type="range" min="0" max="100" step="1" name="musicvolume" value="' +
						this.getMusicVolume() +
						'" />') +
				"</p>";
			buf +=
				'<p class="notif-volume"><label class="optlabel">Notification volume:</label>' +
				(muted
					? "<em>(muted)</em>"
					: '<input type="range" min="0" max="100" step="1" name="notifvolume" value="' +
						this.getNotifVolume() +
						'" />') +
				"</p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="muted"' +
				(muted ? " checked" : "") +
				" /> Mute sounds</label></p>";
		...
		setMute: function (e) {
			...
			if (!muted) {
				this.$(".effect-volume").html(
					'<label class="optlabel">Effect volume:</label><input type="range" min="0" max="100" step="1" name="effectvolume" value="' +
						this.getEffectVolume() +
						'" />',
				);
				this.$(".music-volume").html(
					'<label class="optlabel">Music volume:</label><input type="range" min="0" max="100" step="1" name="musicvolume" value="' +
						this.getMusicVolume() +
						'" />',
				);
				this.$(".notif-volume").html(
					'<label class="optlabel">Notification volume:</label><input type="range" min="0" max="100" step="1" name="notifvolume" value="' +
						this.getNotifVolume() +
						'" />',
				);
			} else {
				this.$(".effect-volume").html(
					'<label class="optlabel">Effect volume:</label><em>(muted)</em>',
				);
				this.$(".music-volume").html(
					'<label class="optlabel">Music volume:</label><em>(muted)</em>',
				);
				this.$(".notif-volume").html(
					'<label class="optlabel">Notification volume:</label><em>(muted)</em>',
				);
			}
	...
	var OptionsPopup = (this.OptionsPopup = Popup.extend({
		...
		events: {
			...
			"change input[name=temporarynotifications]":
		...
		update: function () {
			...
			buf +=
				"<p>" +
				(avatar
					? '<img class="trainersprite" src="' +
						Dex.resolveAvatar(avatar) +
						'" width="40" height="40" style="vertical-align:middle;cursor:pointer" />'
					: "") +
				"<strong>" +
				BattleLog.escapeHTML(name) +
				"</strong></p>";
			buf +=
				'<p><button class="button" name="avatars">Avatar...</button></p>';
			if (!this.editingStatus) {
				buf +=
					'<p><button class="button" name="editstatus">Status...</button></p>';
			} else {
				buf += '<p><input name="statustext" />';
				buf +=
					'<button class="button" name="editstatus"><i class="fa fa-pencil"></i></button></p>';
			}
			if (app.user.get("named")) {
				var registered = app.user.get("registered");
				if (registered && registered.userid === app.user.get("userid")) {
					buf +=
						'<p><button class="button" name="changepassword">Password...</button></p>';
				} else {
					buf +=
						'<p><button class="button" name="register">Register</button></p>';
				}
			}
			...
			if (colorSchemeQuerySupported) {
				buf +=
					'<option value="system"' +
					(theme === "system" ? ' selected="selected"' : "") +
					">Match system theme</option>";
			}
			buf += "</select></label></p>";
			var onePanel = !!Dex.prefs("onepanel");
			if ($(window).width() >= 660) {
				buf +=
					'<p><label class="optlabel">Layout: <select name="onepanel" class="button"><option value=""' +
					(!onePanel ? ' selected="selected"' : "") +
					'>&#x25EB; Left and right panels</option><option value="1"' +
					(onePanel ? ' selected="selected"' : "") +
					">&#x25FB; Single panel</option></select></label></p>";
			}
			buf +=
				'<p><label class="optlabel">Background: <button class="button" name="background">Change background</button></label></p>';
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="noanim"' +
				(Dex.prefs("noanim") ? " checked" : "") +
				" /> Disable animations</label></p>";
			if (navigator.userAgent.includes(" Chrome/64.")) {
				buf +=
					'<p><label class="checkbox"><input type="checkbox" name="nogif"' +
					(Dex.prefs("nogif") ? " checked" : "") +
					" /> Disable GIFs for Chrome 64 bug</label></p>";
			}
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="bwgfx"' +
				(Dex.prefs("bwgfx") ? " checked" : "") +
				" /> Use 2D sprites instead of 3D models</label></p>";
			buf +=
			...
			if (Object.keys(settings).length) {
				buf +=
					'<p><label class="checkbox"><input type="checkbox" name="blockpms"' +
					(settings.blockPMs ? " checked" : "") +
					" /> Block PMs</label></p>";
				buf +=
					'<p><label class="checkbox"><input type="checkbox" name="blockchallenges"' +
					(settings.blockChallenges ? " checked" : "") +
					" /> Block Challenges</label></p>";
			}
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="inchatpm"' +
				(Dex.prefs("inchatpm") ? " checked" : "") +
				" /> Show PMs in chat rooms</label></p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="selfhighlight"' +
				(!Dex.prefs("noselfhighlight") ? " checked" : "") +
				"> Highlight when your name is said in chat</label></p>";

			if (window.Notification) {
				buf +=
					'<p><label class="checkbox"><input type="checkbox" name="temporarynotifications"' +
					(Dex.prefs("temporarynotifications") ? " checked" : "") +
					" /> Notifications disappear automatically</label></p>";
			}
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="leavePopupRoom"' +
				(Dex.prefs("leavePopupRoom") ? " checked" : "") +
				" /> Confirm before leaving a room</label></p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="refreshprompt"' +
				(Dex.prefs("refreshprompt") ? " checked" : "") +
				"> Confirm before refreshing</label></p>";
			...
			buf +=
				'<p><label class="optlabel">Language: <select name="language" class="button">';
			for (var name in possibleLanguages) {
				buf +=
					'<option value="' +
					possibleLanguages[name] +
					'"' +
					(possibleLanguages[name] === curLang
						? ' selected="selected"'
						: "") +
					">" +
					name +
			...
			buf +=
				'<p><label class="optlabel">Tournaments: <select name="tournaments" class="button"><option value="notify"' +
				(tours === "notify" ? ' selected="selected"' : "") +
				'>Notifications</option><option value="nonotify"' +
				(tours === "nonotify" ? ' selected="selected"' : "") +
				'>No Notifications</option><option value="hide"' +
				(tours === "hide" ? ' selected="selected"' : "") +
				">Hide</option></select></label></p>";
			var timestamps = (this.timestamps = Dex.prefs("timestamps") || {});
			buf +=
				'<p><label class="optlabel">Timestamps in chat rooms: <select name="timestamps-lobby" class="button"><option value="off">Off</option><option value="minutes"' +
				(timestamps.lobby === "minutes" ? ' selected="selected"' : "") +
				'>[HH:MM]</option><option value="seconds"' +
				(timestamps.lobby === "seconds" ? ' selected="selected"' : "") +
				">[HH:MM:SS]</option></select></label></p>";
			buf +=
				'<p><label class="optlabel">Timestamps in PMs: <select name="timestamps-pms" class="button"><option value="off">Off</option><option value="minutes"' +
				(timestamps.pms === "minutes" ? ' selected="selected"' : "") +
				'>[HH:MM]</option><option value="seconds"' +
				(timestamps.pms === "seconds" ? ' selected="selected"' : "") +
				">[HH:MM:SS]</option></select></label></p>";
			buf +=
				'<p><label class="optlabel">Chat preferences: <button name="formatting" class="button">Text formatting</button></label></p>';

			if (window.nodewebkit) {
				buf += "<hr />";
				buf += "<p><strong>Desktop app</strong></p>";
				buf +=
					'<p><label class="optlabel"><input type="checkbox" name="logchat"' +
					(Dex.prefs("logchat") ? " checked" : "") +
					"> Log chat</label></p>";
				buf +=
					'<p id="openLogFolderButton"' +
					(Storage.dir ? "" : ' style="display:none"') +
					'><button name="openLogFolder">Open log folder</button></p>';
			}

			buf += "<hr />";
			if (app.user.get("named")) {
				buf +=
					'<p class="buttonbar" style="text-align:right"><button name="login" class="button"><i class="fa fa-pencil"></i> Change name</button> <button name="logout" class="button"><i class="fa fa-power-off"></i> Log out</button></p>';
			} else {
				buf +=
					'<p class="buttonbar" style="text-align:right"><button name="login" class="button">Choose name</button></p>';
			}
			this.$el.html(buf).css("min-width", 160);
		},
	...
	var FormattingPopup = (this.FormattingPopup = Popup.extend({
		...
		initialize: function () {
			...
			var ctrlPlus =
				"<kbd>" +
				(navigator.platform === "MacIntel" ? "Cmd" : "Ctrl") +
				"</kbd> + ";
			buf +=
				"<p>**<strong>bold</strong>** (" + ctrlPlus + "<kbd>B</kbd>)</p>";
			...
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="greentext" ' +
				(cur.hidegreentext ? "checked" : "") +
				' /> Suppress <span class="greentext">&gt;' +
				["meme arrows", "greentext", "quote formatting"][
					Math.floor(Math.random() * 3)
				] +
				"</span></label></p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="me" ' +
				(cur.hideme ? "checked" : "") +
				" /> Suppress <code>/me</code> <em>action formatting</em></label></p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="spoiler" ' +
				(cur.hidespoiler ? "checked" : "") +
				' /> Auto-show spoilers: <span class="spoiler">these things</span></label></p>';
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="links" ' +
				(cur.hidelinks ? "checked" : "") +
				" /> Make [[clickable links]] unclickable</label></p>";
			buf +=
				'<p><label class="checkbox"><input type="checkbox" name="interstice"' +
				(cur.hideinterstice ? "checked" : "") +
				" /> Don't warn for untrusted links</label></p>";
	...
	var AvatarsPopup = (this.AvatarsPopup = Popup.extend({
		...
		initialize: function () {
			...
			var buf = "";
			buf +=
				'<p>Choose an avatar or <button name="close" class="button">Cancel</button></p>';
			...
			for (var i = 1; i <= 293; i++) {
				...
				var offset =
					...
					(((i - 1) % 16) * 80 + 1) +
					"px -" +
					(Math.floor((i - 1) / 16) * 80 + 1) +
					"px";
				buf +=
					'<button name="setAvatar" value="' +
					i +
	...
	var TabListPopup = (this.TabListPopup = Popup.extend({
		...
		renderRooms: function (rooms) {
			...
			for (var i = 0; i < rooms.length; i++)
				buf += app.topbar.renderRoomTab(rooms[i]);
			return buf;
		},
		initialize: function () {
			...
			sideBuf +=
				'<li><a class="button' +
				(curId === "rooms" || curSideId === "rooms" ? " cur" : "") +
				'" href="' +
				app.root +
				'rooms"><i class="fa fa-plus"></i> <span>&nbsp;</span></a></li>';
			if (sideBuf) {
				buf += "<ul>" + sideBuf + "</ul>";
			}
	...
	var CustomBackgroundPopup = (this.CustomBackgroundPopup = Popup.extend({
		...
		initialize: function () {
			...
			buf +=
				'<button name="setBg" value="charizards" class="option' +
				(cur === "charizards" ? " cur" : "") +
				'"><span class="bg" style="background-position:0 -' +
				90 * 0 +
				'px"></span>Charizards</button>';
			buf +=
				'<button name="setBg" value="horizon" class="option' +
				(cur === "horizon" ? " cur" : "") +
				'"><span class="bg" style="background-position:0 -' +
				90 * 1 +
				'px"></span>Horizon</button>';
			buf +=
				'<button name="setBg" value="ocean" class="option' +
				(cur === "ocean" ? " cur" : "") +
				'"><span class="bg" style="background-position:0 -' +
				90 * 3 +
				'px"></span>Ocean</button>';
			buf +=
				'<button name="setBg" value="shaymin" class="option' +
				(cur === "shaymin" ? " cur" : "") +
				'"><span class="bg" style="background-position:0 -' +
				90 * 4 +
				'px"></span>Shaymin</button>';
			buf +=
				'<button name="setBg" value="solidblue" class="option' +
				(cur === "solidblue" ? " cur" : "") +
				'"><span class="bg" style="background: #344b6c"></span>Solid blue</button>';

			buf += '</div><div style="clear:left"></div>';
			buf += "<p><strong>Custom</strong></p>";
			buf +=
				"<p>Drag and drop an image to PS (the background settings don't need to be open), or upload:</p>";
			buf += '<p><input type="file" accept="image/*" name="bgfile"></p>';
			buf += '<p class="bgstatus"></p>';

			buf +=
				'<p><button name="close" class="button"><strong>Done</strong></button></p>';
	...
	CustomBackgroundPopup.readFile = function (file, popup) {
		...
		reader.onload = function (e) {
			if (String(e.target.result).length > 4200000) {
				if (popup) {
					$(".bgstatus").html(
						'<strong style="background:red;color:white;padding:1px 4px;border-radius:4px;display:block">Image is too large and can\'t be saved. It should be under 3.5MB or so.</strong>',
					);
				} else {
					app.addPopupMessage(
						"Image is too large and can't be saved. It should be under 3.5MB or so.",
					);
				}
			} else if (popup) {
				$(".bgstatus").html("Saved");
	...
	var ConfirmBackgroundPopup = (this.ConfirmBackgroundPopup = Popup.extend({
		initialize: function (data) {
			...
			buf +=
				'<p><img src="' +
				data.bgUrl +
				'" style="display:block;margin:auto;max-width:90%;max-height:500px"></p>';
			buf +=
				'<p class="buttonbar"><button name="setBg" value="' +
				data.bgUrl +
				'" class="button"><strong>Change background</strong></button> ';
	...
	var LoginPopup = (this.LoginPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.error) {
				buf +=
					'<p class="error">' + BattleLog.escapeHTML(data.error) + "</p>";
				if (data.error.indexOf("inappropriate") >= 0) {
					...
					buf += "<ol>";
					buf +=
						"<li>Usernames may not impersonate a recognized user (a user with %, @, #, or & next to their name).</li>";
					buf +=
						"<li>Usernames may not be derogatory or insulting in nature, to an individual or group (insulting yourself is okay as long as it's not too serious).</li>";
					buf +=
						"<li>Usernames may not directly reference sexual activity, or be excessively disgusting.</li>";
					buf += "</ol>";
				}
			} else if (data.reason) {
				...
				if (noRenameGames) {
					buf +=
						"<p>You can't change name in the middle of these games:</p>";
					buf += "<ul>" + noRenameGames + "</ul>";
					buf +=
						'<p class="buttonbar"><button type="button" name="force" class="button"><small style="color:red">Forfeit and change name</small></button></p>';
					buf +=
						'<p class="buttonbar"><button type="submit" autofocus class="button"><strong>Cancel</strong></button></p>';
					buf += "</form>";
					this.$el.html(buf);
					return;
				}
			...
			buf +=
				'<p><label class="label">Username: <small class="preview" style="' +
				BattleLog.hashColor(toUserid(name)) +
				'">(color)</small><input class="textbox autofocus" type="text" name="username" value="' +
				BattleLog.escapeHTML(name) +
				'" autocomplete="username"></label></p>';
			if (name) {
				buf +=
					'<p><small>(Others will be able to see your name change. To change name privately, use "Log out")</small></p>';
			}
			buf +=
				'<p class="buttonbar"><button type="submit" class="button"><strong>Choose name</strong></button> <button type="button" name="close" class="button">Cancel</button></p>';
	...
	var ChangePasswordPopup = (this.ChangePasswordPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf +=
				'<p><label class="label">Username: <strong><input type="text" name="username" value="' +
				BattleLog.escapeHTML(app.user.get("name")) +
				'" style="color:inherit;background:transparent;border:0;font:inherit;font-size:inherit;display:block" readonly autocomplete="username" /></strong></label></p>';
			buf +=
				'<p><label class="label">Old password: <input class="textbox autofocus" type="password" name="oldpassword" autocomplete="current-password" /></label></p>';
			buf +=
				'<p><label class="label">New password: <input class="textbox" type="password" name="password" autocomplete="new-password" /></label></p>';
			buf +=
				'<p><label class="label">New password (confirm): <input class="textbox" type="password" name="cpassword" autocomplete="new-password" /></label></p>';
			buf +=
				'<p class="buttonbar"><button type="submit" class="button"><strong>Change password</strong></button> <button type="button" name="close" class="button">Cancel</button></p></form>';
			this.$el.html(buf);
		},
		submit: function (data) {
			$.post(
	...
	var RegisterPopup = (this.RegisterPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf +=
				'<p><label class="label">Username: <strong><input type="text" name="name" value="' +
				BattleLog.escapeHTML(data.name || app.user.get("name")) +
				'" style="color:inherit;background:transparent;border:0;font:inherit;font-size:inherit;display:block" readonly autocomplete="username" /></strong></label></p>';
			buf +=
				'<p><label class="label">Password: <input class="textbox autofocus" type="password" name="password" autocomplete="new-password" /></label></p>';
			buf +=
				'<p><label class="label">Password (confirm): <input class="textbox" type="password" name="cpassword" autocomplete="new-password" /></label></p>';
			buf +=
				'<p><label class="label"><img src="' +
				Dex.resourcePrefix +
				'sprites/gen5ani/pikachu.gif" alt="An Electric-type mouse that is the mascot of the Pok\u00E9mon franchise." /></label></p>';
			buf +=
				'<p><label class="label">What is this pokemon? <input class="textbox" type="text" name="captcha" value="' +
				BattleLog.escapeHTML(data.captcha) +
				'" /></label></p>';
			buf +=
				'<p class="buttonbar"><button type="submit" class="button"><strong>Register</strong></button> <button type="button" name="close" class="button">Cancel</button></p></form>';
			this.$el.html(buf);
		},
		submit: function (data) {
			var name = data.name;
	...
	this.LoginPasswordPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.error) {
				...
				if (data.error.indexOf(" forced you to change ") >= 0) {
					...
					buf +=
						"<li>Usernames may not be derogatory or insulting in nature, to an individual or group (insulting yourself is okay as long as it's not too serious).</li>";
					buf +=
						"<li>Usernames may not reference sexual activity, directly or indirectly.</li>";
					buf +=
						"<li>Usernames may not impersonate a recognized user (a user with %, @, #, or & next to their name).</li>";
					buf += "</ol>";
				}
			...
			buf +=
				'<p><label class="label">Username: <strong><input type="text" name="username" value="' +
				BattleLog.escapeHTML(data.username) +
				'" style="color:inherit;background:transparent;border:0;font:inherit;font-size:inherit;display:block" readonly autocomplete="username" /></strong></label></p>';
			if (data.special === "@gmail") {
				buf +=
					'<div id="g_id_onload" data-client_id="912270888098-jjnre816lsuhc5clj3vbcn4o2q7p4qvk.apps.googleusercontent.com" data-context="signin" data-ux_mode="popup" data-callback="gapiCallback" data-auto_prompt="false"></div>';
				buf +=
					'<div class="g_id_signin" data-type="standard" data-shape="pill" data-theme="filled_blue" data-text="continue_with" data-size="large" data-logo_alignment="left" data-auto_select="true" data-itp_support="true" style="width:fit-content;margin:0 auto">[loading Google log-in button]</div>';
				buf +=
					'<p class="buttonbar"><button name="close" class="button">Cancel</button></p>';
			} else {
				buf +=
					'<p><label class="label">Password: <input class="textbox autofocus" type="password" name="password" autocomplete="current-password" style="width:173px"><button type="button" name="showPassword" aria-label="Show password" style="float:right;margin:-21px 0 10px;padding: 2px 6px" class="button"><i class="fa fa-eye"></i></button></label></p>';
				buf +=
					'<p class="buttonbar"><button type="submit" class="button"><strong>Log in</strong></button> <button type="button" name="close" class="button">Cancel</button></p>';
			}

			buf += '<p class="or">or</p>';
			buf += "<p>If this is someone else's account:</p>";
			buf +=
				'<p class="buttonbar"><button type="button" name="login" class="button">Choose another name</button></p>';
