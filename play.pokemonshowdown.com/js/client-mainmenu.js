	...
	this.MainMenuRoom = this.Room.extend(
		{
			...
			initialize: function () {
				...
				if (app.down) {
					...
					buf +=
						'<p><div style="text-align:center"><img width="96" height="96" src="//play.pokemonshowdown.com/sprites/gen5/teddiursa.png" alt="" class="pixelated" /></div> Bear with us as we freak out.</p>';
					...
					buf += "Partner:<br />";
					buf +=
						'<input class="partnerselect" /><button name="partnersubmit">Invite</button></label></p>';
					buf +=
					...
					buf +=
						'<p><button class="button mainmenu1 big" name="search"><strong>Battle!</strong><br /><small>Find a random opponent</small></button></p></form></div>';
				}
				...
				buf +=
					'<p><button class="button mainmenu5 onlineonly disabled" name="finduser">Find a user</button></p>';
				buf +=
					'<p><button class="button mainmenu6 onlineonly disabled" name="send" value="/friends">Friends</button></p>';
				buf +=
					'<p><button class="button mainmenu7" name="joinRoom" value="resources">Info & Resources</button></p></div>';
			...
			updateChallenge: function ($pmWindow, challenge, name, oName) {
				...
				if (userid !== oUserid) {
					...
					buf +=
						'<p class="buttonbar"><button name="cancelChallenge" class="button">Cancel</button></p></form>';
					$challenge.html(buf);
					return;
				}
				...
				buf +=
					'<p class="buttonbar"><button name="acceptChallenge" class="button"><strong>' +
					BattleLog.escapeHTML(acceptButtonLabel) +
					'</strong></button> <button type="button" name="rejectChallenge" class="button">' +
			...
			clickDatasearchResults: function (e) {
				...
				for (var i = 0; i < results.length; i++) {
					results[i].style.display =
						results[i].style.display === "none" ? "block" : "none";
				}
			},
				...
				if (this.games) {
					...
					for (var roomid in this.games) {
						...
						buf +=
							'<div><a href="/' +
							toRoomid(roomid) +
							'" class="blocklink" style="text-align: center">' +
							BattleLog.escapeHTML(name) +
							"</a></div>";
					}
					buf += "</div>";
					if (!$searchGroup.is(":visible"))
						buf +=
							'<p class="buttonbar"><button name="showSearchGroup" class="button">Add game</button></p>';
					buf += "</form>";
					...
					if (data.challengesFrom[userid]) {
						...
						if (!$challenge.find("button[name=makeChallenge]").length) {
							...
							buf +=
								...
								' /> <abbr title="You can still invite spectators by giving them the URL or using the /invite command">Don\'t allow spectators</abbr></label></p>';
							buf +=
								'<p class="buttonbar"><button name="acceptChallenge" class="button"><strong>Accept</strong></button> <button type="button" name="rejectChallenge" class="button">Reject</button></p></form>';
							$challenge.html(buf);
						...
						if ($challenge.length) {
							...
							if ($acceptButton.length) {
								if ($acceptButton[0].disabled) {
									...
									$challenge.html(
										'<form class="battleform"><p>The challenge was cancelled.</p><p class="buttonbar"><button name="dismissChallenge" class="button">OK</button></p></form>',
									);
								}
							} else if (
			...
			updateFormats: function () {
				if (!window.BattleFormats) {
					...
					$searchForm.append(
						'<p class="cancel buttonbar"><button name="reconnect" class="button">Reconnect</button></p>',
					);
			...
			challenge: function (name, format, team) {
				...
				buf +=
					...
					' /> <abbr title="You can still invite spectators by giving them the URL or using the /invite command">Don\'t allow spectators</abbr></label></p>';
				var bestOfDefault =
					format && BattleFormats[format]
						? BattleFormats[format].bestOfDefault
						: false;
				buf += "<p" + (!bestOfDefault ? ' class="hidden">' : ">");
				buf +=
					'<label class="checkbox"><input type="checkbox" name="bestof" /> <abbr title="Start a team-locked best-of-n series">Best-of-<input name="bestofvalue" type="number" min="3" max="9" step="2" value="3" style="width: 28px; vertical-align: initial;"></abbr></label></p>';
				var teraPreviewDefault =
					format && BattleFormats[format]
						? BattleFormats[format].teraPreviewDefault
						: false;
				buf += "<p" + (!teraPreviewDefault ? ' class="hidden">' : ">");
				buf +=
					'<label class="checkbox"><input type="checkbox" name="terapreview" /> <abbr title="Start a battle with Tera Type Preview">Tera Type Preview</abbr></label></p>';
				buf +=
					'<p class="buttonbar"><button name="makeChallenge" class="button"><strong>Challenge</strong></button> <button type="button" name="dismissChallenge" class="button">Cancel</button></p></form>';
				$challenge.html(buf);
			},
			...
			makeChallenge: function (i, target) {
				...
				var teraPreview = $pmWindow
					.find("input[name=terapreview]")
					.is(":checked");
				if (teraPreview) {
			...
			search: function (i, button) {
				...
				$searchForm
					...
					.addClass("disabled");
				$searchForm.append(
					'<p class="cancel buttonbar"><button name="cancelSearch" class="button">Cancel</button></p>',
				);
	...
	var FormatPopup = (this.FormatPopup = this.Popup.extend({
		...
		initialize: function (data) {
			...
			if (!this.open) {
				...
				this.open = Storage.prefs("openformats") || {
					"S/V Singles": true,
					"S/V Doubles": true,
					"Unofficial Metagames": true,
					"National Dex": true,
					"Ladder Spotlight": true,
			...
			if (!this.starred)
				this.starred = Storage.prefs("starredformats") || {};
			if (!this.search) this.search = "";
			this.onselect = data.onselect;
			this.selectType = data.selectType;
			this.$form = this.sourceEl.closest("form");
			if (!this.selectType)
				this.selectType = this.$form.data("search")
					? "search"
					: "challenge";

			var html =
				'<p><ul class="popupmenu"><li><input name="search" placeholder="Search formats" value="' +
				this.search +
				'" class="textbox autofocus" autocomplete="off" />';
		...
		renderFormats: function () {
			...
			if (this.selectType === "watch" && !this.search) {
				bufs[1] =
					'<li><button name="selectFormat" value="" class="option' +
					(curFormat === "" ? " cur" : "") +
			...
			for (var i in BattleFormats) {
				...
				if (format.section && format.section !== curSection) {
					...
					var open =
						this.open[curSection] || toID(this.search) ? " open" : "";
					bufs[curBuf] +=
						"<p><details" + open + ' section="' + curSection + '">';
					bufs[curBuf] += '<summary><strong style="color: #579">';
					bufs[curBuf] +=
						BattleLog.escapeHTML(curSection) + "</strong></summary>";
				}
		...
		shouldDisplayFormat: function (format) {
			if (this.selectType === "teambuilder") {
				...
				if (format.effectType !== "Format" || format.battleFormat)
					return false;
				if (
					this.selectType !== "watch" &&
					!format[this.selectType + "Show"]
				)
					return false;
			}
			return true;
		...
		selectFormat: function (format) {
			var $form = this.$form.length
				? this.$form
				: this.sourceEl.closest("form");
			...
			if (this.onselect) {
				...
				if ($bestOfCheckbox.length && $bestOfValueInput.length) {
					...
					var bestOfDefault =
						BattleFormats[format] && BattleFormats[format].bestOfDefault;
					if (bestOfDefault) {
	...
	var TeamPopup = (this.TeamPopup = this.Popup.extend(
		{
				...
				if (!teams.length) {
					...
					if (teamFormat) {
						bufs[curBuf] =
							...
							" teams</h3></li>";
						bufs[curBuf] +=
							'<li style="padding-bottom: 5px;"><label class="checkbox"><input type="checkbox"' +
							(this.folderToggleOn ? " checked" : "") +
							"> Group by folders</label></li>";
						for (var i = 0; i < teams.length; i++) {
							if (
								(!teams[i].format && !teamFormat) ||
								teams[i].format === teamFormat
						...
						if (this.folderToggleOn) {
							for (var key in folders) {
								...
								if (!exists) {
									bufs[curBuf] +=
										'<li><button name="selectFolder" class="button" value="' +
										key +
							...
							if (!isNoFolder) {
								bufs[curBuf] +=
									'<li><button name="selectFolder" class="button" value="(No Folder)"><i class="fa fa-folder-open" style="margin-right: 7px; margin-left: 4px;"></i>(No Folder)</button></li>';
							} else {
								bufs[curBuf] +=
									'<li><button name="selectFolder" class="button" value="(No Folder)"><i class="fa fa-folder" style="margin-right: 7px; margin-left: 4px;"></i>(No Folder)</button></li>';
								count++;
								if (
									count % bufBoundary === 0 &&
									count !== 0 &&
			...
			selectFolder: function (key) {
				var keyExists = false;
				var folderNotExpanded = this.folderNotExpanded.filter(
					function (folder) {
						if (folder === key) {
							keyExists = true;
							return false;
