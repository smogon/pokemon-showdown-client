...
export const Dex = new class implements ModdedDex {
	...
	getTeambuilderSprite(pokemon: any, gen = 0) {
		...
		const isKyburmon = kyburmonSprites.includes(pokemon.species);
		if (isKyburmon) {
			spritehtml = `background-image:url(/play.pokemonshowdown.com/${data.spriteDir}${shiny}/${data.spriteid}.png);background-position:${data.x - 5}px ${data.y}px;background-repeat:no-repeat';
			background-size: 90%;`
		}
		else {
