...
import {
	BattleChoiceBuilder,
	type BattleRequestActivePokemon,
	type BattleRequestSideInfo,
	type BattleRequest,
	type BattleMoveRequest,
	type BattleSwitchRequest,
	type BattleTeamRequest,
} from "./battle-choices";
...
class BattlesPanel extends PSRoomPanel<BattlesRoom> {
	...
	static readonly icon = (
		<i class="fa fa-caret-square-o-right" aria-hidden></i>
	);
	...
	applyFilters = (e: Event) => {
		e.preventDefault();
		const minElo = this.base?.querySelector<HTMLInputElement>(
			`select[name=elofilter]`,
		)?.value;
		const searchPrefix = this.base?.querySelector<HTMLInputElement>(
			`input[name=prefixsearch]`,
		)?.value;
	...
	renderBattleLink(battle: BattleDesc) {
		...
		const minEloMessage =
			typeof battle.minElo === "number"
				? `rated ${battle.minElo}`
				: battle.minElo;
		return (
			<div key={battle.id}>
				<a href={`/${battle.id}`} class="blocklink">
					{minEloMessage && (
						<small style="float:right">({minEloMessage})</small>
					)}
					<small>[{format}]</small>
					<br />
					<em class="p1">{battle.p1}</em> <small class="vs">vs.</small>{" "}
					<em class="p2">{battle.p2}</em>
				</a>
			</div>
		);
	}
		...
		return (
			<PSPanelWrapper room={room} scrollable>
				<div class="pad">
					<button
						class="button"
						style="float:right;font-size:10pt;margin-top:3px"
						name="closeRoom"
					>
					...
					<div class="roomlist">
						<p>
							...
							<span
								style={
									Dex.getPokemonIcon("meloetta-pirouette") +
						...
						<label>
							Minimum Elo:{" "}
							<select name="elofilter" onChange={this.applyFilters}>
...
export class BattleRoom extends ChatRoom {
	...
	loadReplay() {
		...
		Net(`https://replay.pokemonshowdown.com/${replayid}.json`)
			.get()
			.catch()
			.then((data) => {
				try {
	...
	override componentDidMount() {
		...
		if (room.battle) {
			this.base!.replaceChild(
				room.battle.scene.$frame![0],
				this.base!.firstChild!,
			);
		}
	}
	override render() {
	...
	render() {
		...
		if (!this.timerInterval && room.battle.kickingInactive) {
			this.timerInterval = setInterval(() => {
				...
				if (
					typeof room.battle.kickingInactive === "number" &&
...
class BattlePanel extends PSRoomPanel<BattleRoom> {
	...
	onKey = (e: KeyboardEvent) => {
		if (e.keyCode === 33) {
			// Pg Up key
			const chatLog = this.base!.getElementsByClassName(
				"chat-log",
			)[0] as HTMLDivElement;
				...
				<p>
					...
					{!room.battle.hardcoreMode && (
						<>
							<button
								class={
									"button button-first" + (atStart ? " disabled" : "")
								}
								data-cmd="/ffto 0"
							...
							<button
								class={
									"button button-last" + (atEnd ? " disabled" : "")
								}
								data-cmd="/ffto +1"
								style="margin-right:2px"
							>
	...
	renderMoveButton(props: {
		...
		if (!props) {
			return (
				<button class="movebutton" disabled>
					&nbsp;
				</button>
			);
		}
		...
		return (
			<div class="movemenu">
				{maybeDisabled && (
					<p>
						<em class="movewarning">
							You <strong>might</strong> have some moves disabled, so you
							won't be able to cancel an attack!
						</em>
					</p>
				)}
				{maybeLocked && (
					<p>
						<em class="movewarning">
							...
							<button class="button" data-cmd="/choose testfight">
								Try Fight button
							</button>{" "}
							{}
				...
				<div class="megaevo-box">
					{canDynamax && (
						<label class={`megaevo${choices.current.max ? " cur" : ""}`}>
							<input
								type="checkbox"
								name="max"
								checked={choices.current.max}
								onChange={this.toggleBoostedMove}
							/>{" "}
							{}
					...
					{canMegaEvo && (
						<label class={`megaevo${choices.current.mega ? " cur" : ""}`}>
							<input
								type="checkbox"
								name="mega"
								checked={choices.current.mega}
								onChange={this.toggleBoostedMove}
							/>{" "}
							{}
							Mega Evolution
					...
					{canUltraBurst && (
						<label
							class={`megaevo${choices.current.ultra ? " cur" : ""}`}
						>
							<input
								type="checkbox"
								name="ultra"
								checked={choices.current.ultra}
								onChange={this.toggleBoostedMove}
							/>{" "}
							{}
							Ultra Burst
					...
					{canZMove && (
						<label class={`megaevo${choices.current.z ? " cur" : ""}`}>
							<input
								type="checkbox"
								name="z"
								checked={choices.current.z}
								onChange={this.toggleBoostedMove}
							/>{" "}
							{}
							Z-Power
					...
					{canTerastallize && (
						<label class={`megaevo${choices.current.tera ? " cur" : ""}`}>
							<input
								type="checkbox"
								name="tera"
								checked={choices.current.tera}
								onChange={this.toggleBoostedMove}
							/>{" "}
							{}
							Terastallize
							...
							<span
								dangerouslySetInnerHTML={{
									__html: Dex.getTypeIcon(canTerastallize),
								}}
							/>
						</label>
					)}
	...
	renderMoveControls(
		...
		const activeIndex =
			battle.mySide.n > 1
				? pokemonIndex + battle.pokemonControlled
				: pokemonIndex;
		const serverPokemon = choices.request.side!.pokemon[pokemonIndex];
		const valueTracker = new ModifiableValue(
			battle,
			battle.nearSide.active[activeIndex]!,
			serverPokemon,
		);
		...
		if (choices.current.max || (active.maxMoves && !active.canDynamax)) {
			...
			return active.moves.map((moveData, i) => {
				...
				const gmaxTooltip = maxMoveData.id.startsWith("gmax")
					? `|${maxMoveData.id}`
					: ``;
	...
	renderMoveTargetControls(
		...
		if (
			(moveTarget === "adjacentAlly" || moveTarget === "adjacentFoe") &&
		...
		const userSlot =
			choices.index() +
			Math.floor(battle.mySide.n / 2) * battle.pokemonControlled;
		const userSlotCross = battle.farSide.active.length - 1 - userSlot;

		return [
			battle.farSide.active
				.map((pokemon, i) => {
					let disabled = false;
					if (
	...
	renderSwitchMenu(
		...
		const maybeTrapped =
			!ignoreTrapping && choices.currentMoveRequest()?.maybeTrapped;
		const trapped =
			!ignoreTrapping &&
			!maybeTrapped &&
			choices.currentMoveRequest()?.trapped;
		...
		return (
			<div class="switchmenu">
				{maybeTrapped && (
					<em class="movewarning">
						You <strong>might</strong> be trapped, so you won't be able to
						cancel a switch!
						<br />
					</em>
				)}
				{trapped && (
					<em class="movewarning">
						You're <strong>trapped</strong> and cannot switch!
						<br />
					</em>
				)}
				{isReviving && (
	...
	renderTeamPreviewChooser(
		request: BattleTeamRequest,
		choices: BattleChoiceBuilder,
	) {
	...
	renderOldChoices(request: BattleRequest, choices: BattleChoiceBuilder) {
		...
		for (let i = 0; i < choices.choices.length; i++) {
			...
			const active =
				request.requestType === "move" ? request.active[i] : null;
			if (choice.choiceType === "move") {
				buf.push(`${pokemon.name} will `);
				if (choice.mega) buf.push(<strong>Mega</strong>, ` Evolve and `);
				if (choice.megax)
					buf.push(<strong>Mega</strong>, ` Evolve (X) and `);
				if (choice.megay)
					buf.push(<strong>Mega</strong>, ` Evolve (Y) and `);
				if (choice.ultra) buf.push(<strong>Ultra</strong>, ` Burst and `);
				if (choice.tera)
					buf.push(
						`Terastallize (`,
						<strong>{active?.canTerastallize || "???"}</strong>,
						`) and `,
					);
				if (choice.max && active?.canDynamax)
					buf.push(
						active?.gigantamax ? `Gigantamax and ` : `Dynamax and `,
					);
				buf.push(
					`use `,
					<strong>{choices.currentMove(choice, i)?.name}</strong>,
				);
		...
		if (choices.isDone()) {
			return (
				<div class="controls">
					...
					<div class="pad">
						{choices.noCancel || room.battle.hardcoreMode ? null : (
							<button data-cmd="/cancel" class="button">
								Cancel
							</button>
						)}
				...
				if (choices.current.move) {
					...
					return (
						<div class="controls">
							<div class="whatdo">
								{this.renderOldChoices(request, choices)}
								{pokemon.name} should use <strong>{moveName}</strong> at
								where? {}
							</div>
							<div class="switchcontrols">
								<div class="switchmenu">
					...
					<div class="controls">
						...
						<div class="switchcontrols">
							<h3 class="switchselect">
								...
								{choices.alreadySwitchingIn.length <= 0
									? `lead`
									: `slot ${choices.alreadySwitchingIn.length + 1}`}
							</h3>
							<div class="switchmenu">
	...
	renderAfterBattleControls() {
		...
		const canUploadReplay = !!window.Config?.server?.registered;
		return (
			<div class="controls">
				<p>
					<span style="float: right">
						...
						>
							<i class="fa fa-download" aria-hidden></i> Download replay
						</a>
						<br />
						<br />
						{canUploadReplay && (
							<button class="button" data-cmd="/savereplay">
								<i class="fa fa-upload" aria-hidden></i> Upload and
								share replay
							</button>
						)}
					</span>
				...
				{room.side ? (
					<p>
						<button class="button" data-cmd="/close">
							...
							<br />
							<small>(closes this battle)</small>
						</button>{" "}
						{}
						<button
							class="button"
							data-cmd={`/closeand /challenge ${room.battle.farSide.id},${room.battle.tier}`}
						>
	...
	handleDownloadReplay = (e: MouseEvent) => {
		...
		// download replay
		let filename = (room.battle.tier || "Battle").replace(
			/[^A-Za-z0-9]/g,
			"",
		);
		let date = new Date();
	...
	override render() {
		...
		const hardcoreStyle = room.battle?.hardcoreMode ? (
			<style
				dangerouslySetInnerHTML={{
					__html: `#${id} .battle .turn, #${id} .battle-history { display: none !important; }`,
				}}
			></style>
		) : null;
		...
		if (room.width < 700) {
			return (
				<PSPanelWrapper room={room} focusClick scrollable="hidden">
					{hardcoreStyle}
					<BattleDiv room={room} />
					<ChatLog
					...
					{room.battle &&
						!room.battle.ended &&
						room.request &&
						room.battle.mySide.id === PS.user.userid && (
