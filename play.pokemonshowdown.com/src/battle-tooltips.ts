...
export class ModifiableValue {
	...
	constructor(battle: Battle, pokemon: Pokemon, serverPokemon: ServerPokemon) {
		...
		this.weatherName = this.battle.dex.moves.get(battle.weather).exists
			? this.battle.dex.moves.get(battle.weather).name
			: this.battle.dex.abilities.get(battle.weather).name;
	}
	...
	modify(factor: number, name?: string) {
		...
		if (
			this.battle.tier.includes("Super Staff Bros") &&
...
export class BattleTooltips {
	...
	static hideTooltip() {
		...
		$("#tooltipwrapper").removeClass(
			"tooltip-locked tooltip-locking-click tooltip-locking-tap",
		);
	}
	...
	static cancelLongTap() {
		...
		$("#tooltipwrapper").removeClass(
			"tooltip-locking-click tooltip-locking-tap",
		);
	}
	...
	holdLockTooltipEvent = (e: JQuery.TriggeredEvent) => {
		...
		BattleTooltips.longTapTimeout = setTimeout(
			...
			isClick
				? BattleTooltips.LONG_CLICK_DELAY
	...
	showTooltip(elem: HTMLElement) {
		...
		switch (type) {
			...
			case "activepokemon": {
				...
				let sideIndex = parseInt(args[1], 10);
				let side =
					this.battle.sides[+this.battle.viewpointSwitched ^ sideIndex];
	...
	getStatusZMoveEffect(move: Dex.Move) {
		...
		let boostText = "";
		if (move.zMove!.boost) {
			boostText = Object.entries(move.zMove!.boost)
				.map(([stat, boost]) => `${BattleTextParser.stat(stat)} +${boost}`)
				.join(", ");
		}
		return boostText;
	}
	...
	showMoveTooltip(
		...
		if (isZOrMax === "zmove") {
			if (item.zMoveFrom === move.name) {
				...
				let moveName =
					BattleTooltips.zMoveTable[item.zMoveType as Dex.TypeName];
		...
		if (this.battle.hardcoreMode) {
			...
			if (move.priority > 1) {
				...
				const hasGaleWingsPriority =
					ability === "galewings" &&
					moveType === "Flying" &&
					(isRelumi || pokemon.hp === pokemon.maxhp);
				if (hasGaleWingsPriority) {
			...
			if (
				...
				let failMessage = hpValues.length
					? `Fails if current HP is ${hpValues.join(" or ")}.`
					: "";
				if (hpValues.includes(serverPokemon.hp))
					failMessage = `<strong class="message-error">${failMessage}</strong>`;
			...
			if (
				...
				const selfKO =
					serverPokemon.maxhp % 4 === 0 ? serverPokemon.maxhp / 4 : null;
				let failMessage = selfKO
					? `KOs yourself if current HP is exactly ${selfKO}.`
					: "";
				if (selfKO === serverPokemon.hp)
					failMessage = `<strong class="message-error">${failMessage}</strong>`;
				if (failMessage) text += `<p>${failMessage}</p>`;
			}
		}
	...
	showPokemonTooltip(
		...
		const ignoreNicks = this.battle.ignoreNicks || this.battle.ignoreOpponent;
		const nickname = ignoreNicks
			? Dex.species.get(pokemon.speciesForme).baseSpecies
			: pokemon.name;
		...
		if (pokemon.fainted && pokemon.maxhp === 100) {
			...
			if (serverPokemon) {
				const status = pokemon.status
					? ` <span class="status ${pokemon.status}">${pokemon.status.toUpperCase()}</span>`
					: "";
		...
		if (serverPokemon) {
			...
			if (clientPokemon?.prevItem) {
				item = "None";
				let prevItem = this.battle.dex.items.get(
					clientPokemon.prevItem,
				).name;
		...
		if (serverPokemon && !isActive) {
			...
			clientPokemon?.side.openTeamSheet &&
	...
	calculateModifiedStats(
		...
		if (
			ability === "hustle" ||
			(ability === "gorillatactics" && !clientPokemon?.volatiles["dynamax"])
		) {
			stats.atk = Math.floor(stats.atk * 1.5);
		}
		if (weather) {
		...
		if (
			item === "eviolite" &&
			this.battle.dex.species.get(serverPokemon.speciesForme).nfe
		) {
			stats.def = Math.floor(stats.def * 1.5);
			stats.spd = Math.floor(stats.spd * 1.5);
		}
		...
		if (this.battle.tier.includes("Super Staff Bros")) {
			...
			if (
				ability === "grassyemperor" &&
				this.battle.hasPseudoWeather("Grassy Terrain")
			) {
				stats.atk = Math.floor(stats.atk * 1.3333);
			}
			if (
				ability === "magicalmysterycharge" &&
			...
			if (
				this.battle.hasPseudoWeather("Anfield Atmosphere") &&
				ability === "youllneverwalkalone"
			) {
	...
	getMoveType(
		...
		if (
			...
			pokemon.getSpeciesForme() === "Terapagos-Stellar"
		) {
			moveType = "Stellar";
		}
		...
		if (this.battle.tier.includes("Super Staff Bros")) {
			if (
				...
				if (
					value.abilityModify(0, "I Can Hear The Heart Beating As One") &&
					moveType === "Normal"
				)
					moveType = "Fairy";
			}
			...
			if (
				move.id === "tsignore" &&
				pokemon.getSpeciesForme().startsWith("Meloetta") &&
			...
			if (move.id === "weatherball" && value.weatherModify(0)) {
				if (
					this.battle.weather === "stormsurge" &&
					item.id !== "utilityumbrella"
				)
					moveType = "Water";
			...
			if (
				move.id === "asongoficeandfire" &&
				pokemon.getSpeciesForme() === "Volcarona"
			)
				moveType = "Ice";
	...
	getMoveAccuracy(move: Dex.Move, value: ModifiableValue, target?: Pokemon) {
		...
		if (this.battle.tier.includes("Super Staff Bros")) {
			...
			if (
				value.tryAbility("Hydrostatic Positivity") &&
				["Electric", "Water"].includes(move.type)
			) {
				accuracyModifiers.push(5325);
			...
			if (
				value.tryAbility("See No Evil, Hear No Evil, Speak No Evil") &&
	...
	getMoveBasePower(
		...
		let variableBPCap = ["crushgrip", "wringout"].includes(move.id)
			? 120
			: move.id === "hardpress"
				? 100
				: undefined;
		if (variableBPCap && target) {
			value.set(
				Math.floor(
					Math.floor(
						(variableBPCap *
							(100 * Math.floor((target.hp * 4096) / target.maxhp)) +
							2048 -
							1) /
							4096,
					) / 100,
		...
		if (
			pokemon.terastallized &&
			(pokemon.terastallized === move.type ||
				pokemon.terastallized === "Stellar") &&
			value.value < 60 &&
			move.priority <= 0 &&
			!move.multihit &&
			!(
		...
		if (this.battle.tier.includes("Super Staff Bros")) {
			if (move.id === "bodycount") {
				value.set(
					50 + 50 * pokemon.side.faintCounter,
			...
			if (move.id === "vengefulmood") {
				value.set(
					Math.min(140, 60 + 20 * pokemon.timesAttacked),
					pokemon.timesAttacked > 0
						? `Hit ${pokemon.timesAttacked} time${pokemon.timesAttacked > 1 ? "s" : ""}`
						: undefined,
				);
			...
			if (
				move.id === "adaptivebeam" &&
				target &&
				Object.values(target.boosts).some((x) => x > 0)
			) {
			...
			if (
				target?.gender === "M" &&
				pokemon.getSpeciesForme().includes("Hearthflame")
			) {
				value.abilityModify(
					1.3,
	...
	getItemBoost(
		...
		if (speciesName === "Ogerpon") {
			...
			if (
				(speciesForme.startsWith("Ogerpon-Wellspring") &&
					itemName === "Wellspring Mask") ||
				(speciesForme.startsWith("Ogerpon-Hearthflame") &&
					itemName === "Hearthflame Mask") ||
				(speciesForme.startsWith("Ogerpon-Cornerstone") &&
					itemName === "Cornerstone Mask")
			) {
				value.itemModify(1.2);
				return value;
			}
			...
			if (clientPokemon.ability) {
				...
				if (species.exists && species.abilities) {
					...
					if (this.battle.rules["Frantic Fusions Mod"]) {
						const fusionSpecies = this.battle.dex.species.get(
							clientPokemon.name,
						);
						if (
							fusionSpecies.exists &&
							fusionSpecies.name !== species.name
						) {
							for (const newAbility of Object.values(
								fusionSpecies.abilities,
							)) {
								if (abilityData.possibilities.includes(newAbility))
									continue;
	...
	getPokemonAbilityText(
		...
		if (!isActive) {
			...
			if (ability)
				text =
					"<small>Ability:</small> " +
					this.battle.dex.abilities.get(ability).name;
		} else {
			if (abilityData.ability) {
				const abilityName = this.battle.dex.abilities.get(
					abilityData.ability,
				).name;
				text = "<small>Ability:</small> " + abilityName;
				const baseAbilityName = this.battle.dex.abilities.get(
		...
		if (
			!text &&
			abilityData.possibilities.length &&
			!hidePossible &&
			!(
				tier.includes("Almost Any Ability") ||
				tier.includes("Hackmons") ||
				tier.includes("Inheritance") ||
				tier.includes("Metronome")
			)
		) {
			text =
...
export class BattleStatGuesser {
	...
	guessEVs(
		...
		plusStat?: Dex.StatNameExceptHP;
		minusStat?: Dex.StatNameExceptHP;
	} {
		if (!set) return {};
		if (role === "?") return {};
		let species = this.dex.species.get(set.species || set.name!);
...
export function BattleStatOptimizer(set: Dex.PokemonSet, formatid: ID) {
	...
	const ignoreEVLimits =
		dex.gen < 3 ||
		((formatid.endsWith("hackmons") || formatid.endsWith("bh")) &&
			dex.gen !== 6) ||
	...
	const plusTooHigh =
		origNature.plus &&
		getStat(origNature.plus, 252, {}) < origStats[origNature.plus];
	...
	if (!minusTooLow) {
		for (const stat of Dex.statNamesExceptHP) {
			if (origStats[stat] < origStats[bestMinus]) {
				...
				if (origNature.minus) {
					savedEVs +=
						(origSpread.evs[origNature.minus] || 0) -
						getMinEVs(origNature.minus, { minus: stat });
				}
				bestMinus = stat;
	...
	if (!plusTooHigh) {
		for (const stat of Dex.statNamesExceptHP) {
			...
			if (
				stat !== origNature.plus &&
				origSpread.evs[stat] &&
				stat !== bestMinus
			) {
				const minEVs = getMinEVs(stat, { plus: stat });
				let plusEVsSaved =
					(origNature.minus === stat
						? getMinEVs(stat, {})
						: origSpread.evs[stat] || 0) - minEVs;
				if (bestPlus && bestPlus !== bestMinus) {
					plusEVsSaved +=
						bestPlusMinEVs! -
						getMinEVs(bestPlus, { plus: stat, minus: bestMinus });
				}
				if (plusEVsSaved > 0 && savedEVs + plusEVsSaved > 0) {
					...
					(plusEVsSaved === 0 && (bestPlus || savedEVs > 0)) ||
					(plusEVsSaved > 0 && savedEVs + plusEVsSaved === 0)
				) {
					if (
						!bestPlus ||
						getStat(stat, getMinEVs(stat, { plus: stat }), {
							plus: stat,
						}) > origStats[stat]
					) {
						savedEVs += plusEVsSaved;
	...
	if (bestPlus && savedEVs >= 0) {
		...
		if (bestPlus !== origNature.plus || bestMinus !== origNature.minus) {
			...
			if (
				origNature.plus &&
				origNature.plus !== bestPlus &&
				origNature.plus !== bestMinus
			) {
				newSpread.evs[origNature.plus] = getMinEVs(
					origNature.plus,
					newSpread,
				);
			}
			if (
				origNature.minus &&
				origNature.minus !== bestPlus &&
				origNature.minus !== bestMinus
			) {
				newSpread.evs[origNature.minus] = getMinEVs(
					origNature.minus,
					newSpread,
				);
			}
			...
			if (
				Math.floor(getStat(bestPlus, bestMinusMinEVs!, newSpread) / 11) <=
				...
				savedEVs +=
					(origSpread.evs[origNature.plus] || 0) -
					newSpread.evs[origNature.plus]!;
				savedEVs +=
					(origSpread.evs[origNature.minus] || 0) -
					newSpread.evs[origNature.minus]!;
