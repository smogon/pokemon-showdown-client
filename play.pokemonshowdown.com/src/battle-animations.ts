...
export class BattleScene implements BattleSceneStub {
	...
	pos(loc: ScenePos, obj: SpriteData) {
		...
		// When using upstream sprites, revert to standard Showdown scaling.
		const useUpstream = Dex.prefs('useupstreamsprites');
		let scale = (obj.gen === 5 ?
			2.0 - ((loc.z!) / 200) :
			obj.isFrontSprite ? 1.5 - 0.5 * ((loc.z!) / 200) : (useUpstream ? 1.5 - 0.5 * ((loc.z!) / 200) : 1.0));
	...
	updateGen() {
		...
		if (typeof rated === 'string' && rated.startsWith("Smogon Premier League")) {
			...
			if (gen <= 1) bg = 'fx/bg-gen1.png?';
				...
				// .png backgrounds are gen5-style (in fx/), .jpg are gen6-style (in sprites/gen6bgs/)
				bg = relumiBg.includes('.png') ? `fx/${relumiBg}` : `sprites/gen6bgs/${relumiBg}`;
