	...
	fixPrefs(newPrefs: any) {
		...
		if (oldShowjoins !== undefined && typeof oldShowjoins !== "object") {
			const showjoins: { [serverid: string]: { [roomid: string]: 1 | 0 } } =
				{};
			const serverShowjoins: { [roomid: string]: 1 | 0 } = {
				global: oldShowjoins ? 1 : 0,
			};
			const showroomjoins = newPrefs["showroomjoins"] as {
		...
		const colorSchemeQuerySupported =
	...
	doAutojoin() {
		...
		if (autojoin) {
			...
			for (let title of rooms.split(",")) {
				PS.addRoom({
					id: toID(title) as string as RoomID,
					title,
					connected: true,
					autofocus: false,
				});
...
class PSTeams extends PSStreamModel<"team" | "format"> {
	...
	unpackOldBuffer(buffer: string) {
		PS.alert(
			`Your team storage format is too old for PS. You'll need to upgrade it at https://${Config.routes.client}/recoverteams.html`,
		);
		this.list = [];
	}
	packAll(teams: Team[]) {
		return teams
			.map(
				(team) =>
					...
					(team.format || team.isBox
						? `${team.format || ""}${team.isBox ? "-box" : ""}]`
						: ``) +
	...
	unpackLine(line: string): Team | null {
		...
		const teamid =
			leftBracketIndex > 0
				? Number(line.slice(0, leftBracketIndex))
				: undefined;
		return {
			name,
			format: format as ID,
	...
	loadRemoteTeams() {
		PSLoginServer.query("getteams").then((data) => {
			...
			if (data.actionerror) {
				return PS.alert(
					"Error loading uploaded teams: " + data.actionerror,
				);
			}
			...
			for (const team of Object.values(teams)) {
				...
				for (const localTeam of this.list) {
					...
					if (compare === "rename") {
						if (!localTeam.name.endsWith(" (local version)"))
				...
				if (!matched) {
					const mons = team.team
						.split(",")
						.map((m: string) => ({ species: m, moves: [] }));
					const newTeam: Team = {
						name: team.name,
						format: team.format,
						folder: "",
	...
	loadTeam(
		...
		if (!team?.uploaded || team.uploadedPackedTeam)
			return ifNeeded ? undefined : Promise.resolve();
		if (team.uploaded.notLoaded && team.uploaded.notLoaded !== true)
			return team.uploaded.notLoaded;
		...
		return (team.uploaded.notLoaded = PSLoginServer.query("getteam", {
			...
			if (!data?.team) {
				PS.alert(
					`Failed to load team: ${data?.actionerror || "Error unknown. Try again later."}`,
				);
				return;
			}
	...
	compareTeams(serverTeam: UploadedTeam, localTeam: Team) {
		...
		let sanitize = (name: string) =>
			(name || "").replace(/\s+\(server version\)/g, "").trim();
		const nameMatches =
			sanitize(serverTeam.name) === sanitize(localTeam.name);
		...
		const otherMons = Teams.unpackSpeciesOnly(localTeam.packedTeam)
			.map(toID)
			.sort()
			.join(",");
...
class PSUser extends PSStreamModel<PSLoginState | null> {
	...
	validateName(name: string): string {
		...
		for (const i in replaceList) {
			replaceRegexes.push([
				new RegExp("[" + replaceList[i as "A"] + "]", "g"),
				i,
			]);
		}
		const normalizeRegexes: [RegExp, string][] = [];
		for (const i in normalizeList) {
			normalizeRegexes.push([
				new RegExp("[" + normalizeList[i as "A"] + "]", "g"),
				i,
			]);
		}
	...
	changeName(name: string) {
		...
		this.loggingIn = name;
		this.update(null);
		PSLoginServer.rawQuery("getassertion", {
			userid,
	...
	changeNameWithPassword(
		name: string,
		password: string,
		special: PSLoginState = { needsPassword: true },
	) {
		this.loggingIn = name;
		if (!password && !special) {
			this.updateLogin({
...
class PSServer {
	...
	prefix = "/showdown";
	protocol: "http" | "https" = Config.defaultserver.httpport
		? "https"
		: "http";
...
export function makeLoadTracker() {
	let resolver: () => void;
	const tracker: Promise<void> & { loaded: () => void } = new Promise<void>(
		(resolve) => {
			resolver = resolve;
		},
	) as any;
...
export class PSRoom extends PSStreamModel<Args | null> implements RoomOptions {
	...
	onParentEvent:
		| ((eventId: "focus" | "keydown", e?: Event) => false | void)
		| null = null;
	...
	notify(options: {
		...
		if (!roomIsFocused) {
			...
			try {
				desktopNotification = new Notification(options.title, {
					body: options.body,
				});
	...
	subtleNotify() {
		...
		const lastSeenTimestamp =
			PS.prefs.logtimes?.[PS.server.id]?.[this.id] || 0;
		const lastMessageTime = +(room.lastMessage?.[1] || 0);
		this.isSubtleNotifying = !(
			lastMessageTime + room.timeOffset <=
			lastSeenTimestamp
		);
		PS.update();
	}
	...
	autoDismissNotifications() {
		...
		if (room.lastMessageTime) {
			...
			if (!lastMessageDates[PS.server.id])
				lastMessageDates[PS.server.id] = {};
	...
	receiveLine(args: Args): void {
		switch (args[0]) {
			...
			case "notify": {
				...
				if (toHighlight && !ChatRoom.getHighlight(toHighlight, this.id))
					break;
	...
	add(line: string, ifChat?: boolean) {
		if (this.type !== "chat" && this.type !== "battle") {
			if (!ifChat) {
				...
				PS.rooms["dm-" as RoomID]?.receiveLine(
	...
	parseClientCommands(commands: ClientCommands<this>) {
		...
		for (const cmd in commands) {
			...
			for (const name of names) {
				if (name.includes(" "))
					throw new Error(
						`Client command names cannot contain spaces: ${name}`,
					);
	...
	globalClientCommands = this.parseClientCommands({
		"j,join"(target, cmd, elem) {
			...
			const roomid = /[^a-z0-9-]/.test(target)
				? (toID(target) as any as RoomID)
				: (target as RoomID);
		...
		"part,leave,close"(target, cmd, elem) {
			...
			if (
				room?.type === "battle" &&
				!battle.ended &&
				room.users[PS.user.userid]?.startsWith("☆") &&
				!battle.isReplay
			) {
		...
		"reconnect,connect"() {
			...
			if (uptime > 24 * 60 * 60 * 1000) {
				PS.confirm(
					`It's been over a day since you first connected. Please refresh.`,
					{
						okButton: "Refresh",
					},
				).then((confirmed) => {
		...
		ignore(target) {
			...
			if (toID(target) === PS.user.userid) {
				...
				this.add(
					`||User '${target}' is already on your ignore list. ` +
						`(Moderator messages will not be ignored.)`,
				);
			} else {
				ignore[toID(target)] = 1;
				this.add(
					`||User '${target}' ignored. (Moderator messages will not be ignored.)`,
				);
		...
		clearignore(target) {
			if (toID(target) !== "confirm") {
				...
				this.add(
					"|html|If you're sure, use <code>/clearignore confirm</code>",
				);
				return false;
			}
			let ignoreList = PS.prefs.ignore || {};
			if (!Object.keys(ignoreList).length)
				return this.add("You have no ignored users.");
		...
		ignorelist(target) {
			...
			if (ignoreList.length === 0) {
				...
				if (!ignoring.length)
					return this.add("||You are currently not ignoring anyone.");
		...
		afd(target) {
			...
			if (mode === "sprites") {
				...
				PS.prefs.set("afd", null);
				this.add(
					"||April Fools' Day mode set to DEFAULT (Currently " +
						(Dex.afdMode ? "FULL" : "OFF") +
						").",
				);
			} else if (mode === "full") {
				...
				if (Config.server?.afd) {
					this.add(
						"||You're using the AFD URL, which will still override this setting and enable AFD mode on refresh.",
					);
				}
			} else {
				if (target)
					this.add('||AFD option "' + target + '" not recognized');
				...
				if (!curMode)
					curMode =
						"DEFAULT (currently " + (Dex.afdMode ? "FULL" : "OFF") + ")";
				this.add("||AFD is currently set to " + mode);
				this.send("/help afd");
			}
			for (let roomid in PS.rooms) {
				let battle =
					PS.rooms[roomid] && (PS.rooms[roomid] as BattleRoom).battle;
				if (!battle) continue;
				battle.resetToCurrentTurn();
			}
		},
		clearpms() {
			let rooms = PS.miniRoomList.filter((roomid) =>
				roomid.startsWith("dm-"),
			);
			if (!rooms.length)
				return this.add("||You do not have any PM windows open.");
			...
			if (target.includes(" ")) {
						...
						for (let i = 0, len = targets.length; i < len; i++) {
							...
							if (/[\\^$*+?()|{}[\]]/.test(targets[i])) {
								...
								try {
									...
									return this.add(
										`|error|${e.message.substr(0, 28) === "Invalid regular expression: " ? e.message : "Invalid regular expression: /" + targets[i] + "/: " + e.message}`,
									);
								}
							}
							if (highlightList.includes(targets[i])) {
								return this.add(
									`|error|${targets[i]} is already on your highlights list.`,
								);
							}
						}
						highlights[key] = highlightList.concat(targets);
						this.add(
							`||Now highlighting on ${key === "global" ? "(everywhere): " : "(in " + key + "): "} ${highlights[key].join(", ")}`,
						);
						// We update the regex
				...
				if (["clear", "roomclear", "clearall"].includes(target)) {
					...
					if (key) {
						highlights[key] = [];
						this.add(
							`||All highlights (${key === "global" ? "everywhere" : "in " + key}) cleared.`,
						);
						ChatRoom.updateHighlightRegExp(highlights);
					} else {
						PS.prefs.set("highlights", null);
						this.add(
							"||All highlights (in all rooms and globally) cleared.",
						);
						ChatRoom.updateHighlightRegExp({});
					}
				} else if (
					["show", "list", "roomshow", "roomlist"].includes(target)
				) {
					// Shows a list of the current highlighting words
					let key = target.startsWith("room")
						? PS.server.id + "#" + this.id
						: "global";
					if (highlights[key] && highlights[key].length > 0) {
						this.add(
							`||Current highlight list ${key === "global" ? "(everywhere): " : "(in " + key + "): "}${highlights[key].join(", ")}`,
						);
					} else {
						this.add(
							`||Your highlight list${key === "global" ? "" : " in " + key} is empty.`,
						);
					}
				} else {
					// Wrong command
		...
		"h,help"(target) {
			switch (toID(target)) {
				...
				case "hidejoins":
					this.add(
						"||/showjoins [room] - Receive users' join/leave messages. Optionally for only specified room.",
					);
					this.add(
						"||/hidejoins [room] - Ignore users' join/leave messages. Optionally for only specified room.",
					);
					return;
				case "showbattles":
				case "hidebattles":
					this.add(
						"||/showbattles - Receive links to new battles in Lobby.",
					);
					this.add(
						"||/hidebattles - Ignore links to new battles in Lobby.",
					);
					return;
				case "ffto":
				case "fastforwardto":
					this.add(
						"||/ffto [turn] - Skip to turn [turn] in the current battle.",
					);
	...
	handleSend(line: string, element = this.currentElement) {
		...
		const cmd = (
			spaceIndex >= 0 ? line.slice(1, spaceIndex) : line.slice(1)
		) as "parsed";
		const target = spaceIndex >= 0 ? line.slice(spaceIndex + 1).trim() : "";

		const cmdHandler =
			this.globalClientCommands[cmd] || this.clientCommands?.[cmd];
	...
	sendDirect(msg: string) {
		if (this.connected === "expired")
			return this.add(
				`This room has expired (you can't chat in it anymore)`,
			);
		PS.send(msg, this.id);
	}
	destroy() {
...
type PSRoomPanelSubclass<T extends PSRoom = PSRoom> =
	(new () => PSRoomPanel<T>) & {
		readonly id: string;
...
export const PS = new (class extends PSModel {
	...
	constructor() {
		super();
		console.log("[PS] Initializing PS object");
	}
	...
	newsHTML =
		document.querySelector("#room-news .readable-bg")?.innerHTML || "";
	newsId =
		document.getElementById("room-news")?.getAttribute("data-newsid") || null;
	...
	constructor() {
		...
		if (autojoin) {
			...
			for (let title of rooms.split(",")) {
				this.addRoom({
					id: toID(title) as unknown as RoomID,
					title,
					connected: true,
					autofocus: false,
				});
	...
	getRoom(
		elem: HTMLElement | EventTarget | null | undefined,
		skipClickable?: boolean,
	): PSRoom | null {
		let curElem: HTMLElement | null = elem as HTMLElement;
		// might be the close button on the roomtab
		if (
			(curElem as HTMLButtonElement)?.name === "closeRoom" &&
			(curElem as HTMLButtonElement).value
		) {
		...
		while (curElem) {
			...
			if (curElem.getAttribute("data-roomid")) {
				return (
					PS.rooms[curElem.getAttribute("data-roomid") as RoomID] || null
				);
			}
			if (
				skipClickable &&
				(curElem.tagName === "A" ||
					curElem.tagName === "BUTTON" ||
					curElem.tagName === "INPUT" ||
					curElem.tagName === "SELECT" ||
					curElem.tagName === "TEXTAREA" ||
					curElem.tagName === "LABEL" ||
					curElem.classList?.contains("textbox") ||
					curElem.classList?.contains("username"))
			) {
				return null;
			}
	...
	dragOnto(
		fromRoom: PSRoom,
		toLocation: "left" | "right" | "mini-window",
		toIndex: number,
	) {
			...
			switch (args[0]) {
				...
				case "nametaken": {
					PS.join("login" as RoomID, {
						args: {
							error: `Someone is already using the name ${args[1]}.`,
						},
					});
					break;
				}
	...
	calculateLeftPanelWidth() {
		...
		if (excess >= 0) {
			...
			return (
				left.width +
				Math.floor((excess * leftStretch) / (leftStretch + rightStretch))
			);
		}
	...
	getRouteLocation(roomid: RoomID): PSRoomLocation {
		...
		if (routeInfo.startsWith("*"))
			return routeInfo.slice(1) as PSRoomLocation;
	...
	updateRoomTypes() {
		...
		for (const roomid in this.rooms) {
			...
			const typeIsGuessed =
				room.type === this.routes["*"] && !roomid.includes("-");
			if (!room.isPlaceholder && !typeIsGuessed) continue;

			let type =
				(!typeIsGuessed && room.type) ||
				this.getRoute(roomid as RoomID) ||
				room.type ||
				"";
	...
	horizontalNav(room = this.room) {
		...
		const miniRoom =
			this.miniRoomList[0] !== "news" ? this.miniRoomList[0] : null;
	...
	confirm(
		...
		opts: {
			okButton?: string;
			cancelButton?: string;
			otherButtons?: preact.ComponentChildren;
	...
	prompt(
		...
		opts: {
			...
			okButton?: string;
			cancelButton?: string;
			type?: "text" | "password" | "number" | "numeric";
			otherButtons?: preact.ComponentChildren;
			parentElem?: HTMLElement | null;
		} = {},
	): Promise<string | null> {
	 ...
	 */
	addRoom(
		options: RoomOptions & { autoclosePopups?: boolean; autofocus?: boolean },
	) {
		...
		if (options.id.startsWith("battle-") && PS.prefs.rightpanelbattles)
			options.location = "right";
		options.parentRoomid ??= this.getRoom(options.parentElem)?.id;
		const parentRoom = options.parentRoomid
			? this.rooms[options.parentRoomid]
			: null;
	...
	roomVisible(room: PSRoom): boolean {
		if (PS.isNormalRoom(room)) {
			return !this.leftPanelWidth
				? room === this.panel
				: room === this.leftPanel || room === this.rightPanel;
		}
		if (room.location === "mini-window") {
			return !this.leftPanelWidth
				? this.mainmenu === this.panel
				: this.mainmenu === this.leftPanel;
		}
		// some kind of popup
		return true;
	}
	...
	isPopup(room: PSRoom | undefined | null) {
		...
		return (
			room.location === "popup" ||
			room.location === "semimodal-popup" ||
			room.location === "modal-popup"
		);
	}
	...
	moveRoom(
		...
		if (room.location === "mini-window") {
			...
			if (this.room === room)
				this.room = this.rooms["rooms"] || this.leftPanel;
			if (this.panel === room)
				this.panel = this.rooms["rooms"] || this.leftPanel;
			if (this.rightPanel === room)
				this.rightPanel = this.rooms["rooms"] || null;
		}
		...
		switch (location) {
			case "left":
				this.leftRoomList.splice(
					Math.max(index ?? Infinity, 1),
					0,
					room.id,
				);
				break;
			case "right":
				this.rightRoomList.splice(
					Math.min(index ?? -1, this.rightRoomList.length - 1),
					0,
					room.id,
				);
				break;
			...
			default:
				throw new Error(
					`Invalid room location: ${location satisfies never as string}`,
				);
		}
		if (!background) {
			...
			if (location === "mini-window")
				this.leftPanel = this.panel = this.mainmenu;
			this.room = room;
		}
	}
	removeRoom(room: PSRoom) {
		...
		if (PS.rightPanel === room) {
			let newRightRoomid =
				PS.rightRoomList[rightRoomIndex] ||
				PS.rightRoomList[rightRoomIndex - 1];
		...
		if (room.location === "mini-window") {
			...
			if (PS.room === room) {
				PS.room =
					PS.rooms[PS.miniRoomList[miniRoomIndex]] ||
					PS.rooms[PS.miniRoomList[miniRoomIndex - 1]] ||
					PS.mainmenu;
			}
		}
		...
		if (
			this.popups.length &&
			room.id === this.popups[this.popups.length - 1]
		) {
			this.popups.pop();
			if (this.popups.length) {
				...
				if (PS.room.location !== "mini-window" || PS.panel !== PS.mainmenu)
					PS.room = PS.panel;
			}
		}
	...
	updateAutojoin() {
		...
		for (let roomid of rooms) {
			...
			autojoins.push(
				room.id.includes("-") ? room.id : room.title || room.id,
			);
			if (
				room.id === "staff" ||
				room.id === "upperstaff" ||
				(PS.server.id !== "showdown" && room.id === "lobby")
			)
				continue;
			autojoinCount++;
	...
	playNotificationSound() {
		if (window.BattleSound && !this.prefs.mute) {
			window.BattleSound.playSound(
				"audio/notification.wav",
				this.prefs.notifvolume,
			);
		}
	}
})();
