...
class BattleTextParser {
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-start': {
			...
			if (id.startsWith('protosynthesis') || id.startsWith('quarkdrive') || id.startsWith('stealthadvantage')) {
		...
		case '-activate': {
			...
			if ((id === 'protosynthesis' || id === 'quarkdrive' || id === 'stealthadvantage') && kwArgs.fromitem) {
		...
		case '-fail': {
			...
			templateId = 'fail';
			if (['brn', 'frz', 'par', 'psn', 'slp', 'frb', 'substitute', 'shedtail'].includes(id)) {
