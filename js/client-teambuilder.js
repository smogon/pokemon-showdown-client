(function (exports, $) {
	...
	var TeambuilderRoom = exports.TeambuilderRoom = exports.Room.extend({
		...
		arrangeCallback: {
			...
			item: function (item) {
				if (!item) return ['Usable Items', 'Items'];
				
				if (item.isViable) { return 'Usable Items'; }
				else
				{
					...
					if (this.curSet)
					{
						...
						if (item.megaEvolves && item.megaEvolves === template.baseSpecies) return 'Usable Items';
						
						//Only NFEs can use Eviolite
						if (item.id === 'eviolite' && template.evos) return 'Usable Items';
						
						//These items are only usable by specific Pokemon
						if (item.onSwitchIn)
						{
							if (item.id === 'redorb' && (template.baseSpecies === 'Groudon')) return 'Usable Items';
							if (item.id === 'blueorb' && (template.baseSpecies === 'Kyogre')) return 'Usable Items';
							if (item.id === 'griseousorb' && (template.baseSpecies === 'Giratina')) return 'Usable Items';
						}
						if (item.id === 'thickclub' && (this.curSet.species === 'Marowak' || this.curSet.species === 'Cubone')) return 'Usable Items';
						if (item.id === 'stick' && (this.curSet.species === 'Farfetch\'d')) return 'Usable Items';
						if (item.id === 'souldew' && (this.curSet.species === 'Latios' || this.curSet.species === 'Latias')) return 'Usable Items';
						if (item.id === 'lightball' && (template.baseSpecies === 'Pikachu')) return 'Usable Items';
						if (this.curSet.species === 'Clamperl' && (item.id === 'deepseascale' || item.id === 'deepseascale')) return 'Usable Items';
						
						//Only berries that reduce super effective damage for this Pokemon are usable
						if (item.isBerry && item.onSourceModifyDamage && item.id !== 'enigmaberry')
						{
							...
							if (weakness > 0) return 'Usable Items';
						}
					}
				}
				return 'Items';
