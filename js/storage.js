	...
	load: function (bgUrl, bgid, hues) {
		...
		if (!hues) {
			...
			case 'shaymin':
				...
				break;
			case 'charizards':
				hues = ["250,40%", "255,40%", "260,40%", "265,40%", "270,40%", "275,40%"];
				break;
			case 'psday':
...
Storage.packTeam = function (team) {
	...
	for (var i = 0; i < team.length; i++) {
		...
		if (set.pokeball || (set.hpType && !hasHP) || set.gigantamax || (set.dynamaxLevel !== undefined && set.dynamaxLevel !== 10) || set.teraType || set.cmType) {
			buf += ',' + (set.hpType || '');
...
Storage.importTeam = function (buffer, teams) {
	...
	for (var i = 0; i < text.length; i++) {
		...
		} else if (line.substr(0, 13) === 'Custom Move: ') {
			line = line.substr(13);
			curSet.cmType = line;
