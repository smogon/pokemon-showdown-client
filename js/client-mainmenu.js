(function ($) {
	...
	this.MainMenuRoom = this.Room.extend({
		...
		parseChatMessage: function (message, name, timestamp, isHighlighted, $chatElem, isNotPM) {
			...
			var clickableName = '<small>' + BattleLog.escapeHTML(group) + '</small><span class="username" data-roomgroup="' + BattleLog.escapeHTML(group) + '" data-name="' + BattleLog.escapeHTML(name) + '">' + (BattleLog.escapeHTML(name).includes("pokebridge") ? "" : BattleLog.escapeHTML(name)) + '</span>';
