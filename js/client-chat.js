(function ($) {
	...
	var UserList = this.UserList = Backbone.View.extend({
		...
		constructItem: function (userid) {
			...
			console.log(BattleLog.escapeHTML(userid) === 'yoshifanfic' ? 'style="background-image:url(https://media.tenor.com/p_IwfyFVRGkAAAAd/sableye-pokemon.gif);' : '');
			console.log(userid);
			text += '<li' + (this.room.userForm === userid ? ' class="cur"' : '') + ' id="' + this.room.id + '-userlist-user-' + BattleLog.escapeHTML(userid) + '"' + BattleLog.escapeHTML(userid) === 'yoshifanfic' ? 'style="background-image:url(https://media.tenor.com/p_IwfyFVRGkAAAAd/sableye-pokemon.gif);' : '' + '>';
