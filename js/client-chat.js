		...
		parseCommand: function (text) {
			...
			case 'highlight':
				...
				if (target.indexOf(',') > -1) {
					...
					case 'roomadd':
						...
						var hlkey = (thisroom === 'global' ? 'global' : Config.server.id + '#' + thisroom);
						...
						for (var i = 1, len = targets.length; i < len; i++) {
							...
							if (highlights[hlkey].indexOf(targets[i]) > -1) {
					...
					case 'roomdelete':
						...
						var newHls = [];
						for (var i = 0, len = highlights[hlkey].length; i < len; i++) {
							if (targets.indexOf(highlights[hlkey][i]) === -1) {
								newHls.push(highlights[hlkey][i]);
							}
						}
					...
					if (target === 'delete' || target === 'roomdelete') {
						var hlkey = (target === 'delete' ? 'global' : (this.type === 'battle' ? Config.server.id + '#' + 'battle' : Config.server.id + '#' + this.id));
						if (!highlights[hlkey]) highlights[hlkey] = [];
						...
						if (highlights[hlkey].length > 0) {
							this.add('Current' + textending + ': ' + highlights[hlkey].join(", "));
						} else {
			...
			case 'help':
				...
				case 'hl':
					...
					this.add('/highlight add, [word] - Add the word [word] to your global highlight list.');
					this.add('/highlight list - List all words that currently highlight you.');
					this.add('/highlight delete, [word] - Delete the word [word] from your global highlight list.');
					this.add('/highlight delete - Clear your global highlight list.');
					this.add('/highlight roomadd, [word] - Add the word [word] to your room-specific highlight list.');
					return false;
				case 'rank':
				case 'ranking':
				case 'rating':
