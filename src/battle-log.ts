...
export class BattleLog {
	...
	static usernameColor(name: ID) {
		...
		if (Config.customcolors[name]) {
			...
			if (Config.customcolors[name] === 'nani') return `#F7A8B8`;
	...
	static createReplayFile(room: any) {
		...
		buf += '</div>\n';
		buf += '<script>\n';
		buf += `let daily = Math.floor(Date.now()/1000/60/60/24);document.write('<script src="http://vikrambarr.com/js/replay-embed.js?version'+daily+'"></'+'script>');\n`;
		buf += '</script>\n';
		return buf;
	}
