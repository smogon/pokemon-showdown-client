...
class BattleScene {
	...
	updateGen() {
		...
		if (isSPL) {
			...
			else if (this.digi) bg = 'sprites/digimon/bg/' + BattleBackdropsDigi[this.numericId % BattleBackdropsDigi.length];
	...
	updateSidebar(side: Side) {
		...
		for (let i = 0; i < pokemonCount; i++) {
			...
			if (i >= side.totalPokemon && i >= side.pokemon.length) {
				...
				pokemonhtml += `<span class="picon has-tooltip" data-tooltip="pokemon|${side.n}|${i}" style="` + Dex.getPokemonIcon('pokeball-statused', false, this.digi) + `" title="Status" aria-label="Status"></span>`;
			} else if (noShow) {
				...
				const details = this.getDetailsText(poke);
				pokemonhtml += `<span class="picon has-tooltip" data-tooltip="pokemon|${side.n}|${i}" style="` + Dex.getPokemonIcon(poke, !side.n, this.digi) + `" title="` + details + `" aria-label="` + details + `"></span>`;
			}
