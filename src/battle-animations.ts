...
class BattleScene {
	...
	constructor(battle: Battle, $frame: JQuery, $logFrame: JQuery) {
		...
		this.log.battleParser!.pokemonName = (pokemonId: string) => {
			...
			if (battle.ignoreNicks || battle.ignoreOpponent) {
				...
				if (pokemon) return pokemon.speciesName;
			}
	...
	getDetailsText(pokemon: Pokemon) {
		let name = pokemon.side?.n &&
			(this.battle.ignoreOpponent || this.battle.ignoreNicks) ? pokemon.speciesName : pokemon.name;
		if (name !== pokemon.speciesName) {
	...
	teamPreview() {
		...
		for (let siden = 0; siden < 2; siden++) {
			...
			for (let i = 0; i < side.pokemon.length; i++) {
				...
				if (pokemon.speciesName === 'Ludicolo') ludicoloCount++;
				if (pokemon.speciesName === 'Lombre') lombreCount++;
...
class PokemonSprite extends Sprite {
	...
	animTransform(pokemon: Pokemon, isCustomAnim?: boolean, isPermanent?: boolean) {
		...
		if (!this.scene.animating) return;
		let speciesid = toID(pokemon.getSpeciesName());
		let doCry = false;
	...
	dogarsCheck(pokemon: Pokemon) {
		...
		if (pokemon.speciesName === 'Koffing' && pokemon.name.match(/dogars/i)) {
			this.scene.setBgm(-1);
	...
	getStatbarHTML(pokemon: Pokemon) {
		...
		buf += `<strong>${BattleLog.escapeHTML(ignoreNick ? pokemon.speciesName : pokemon.name)}`;
		const gender = pokemon.gender;
		if (gender === 'M' || gender === 'F') {
		...
		let symbol = '';
		if (pokemon.speciesName.indexOf('-Mega') >= 0) symbol = 'mega';
		else if (pokemon.speciesName === 'Kyogre-Primal') symbol = 'alpha';
		else if (pokemon.speciesName === 'Groudon-Primal') symbol = 'omega';
		if (symbol) {
