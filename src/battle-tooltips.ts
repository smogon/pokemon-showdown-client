...
class BattleTooltips {
				...
				if (weather === 'sunnyday' || weather === 'desolateland') {
					...
					if (allyActive) {
						for (const ally of allyActive) {
							...
							if (allyAbility === 'Flower Gift' && (ally.getSpecies().baseSpecies === 'Noivern' || ally.getSpecies().baseSpecies === 'Cherrim' || this.battle.gen <= 4)) {
								stats.atk = Math.floor(stats.atk * 1.5);
								stats.spd = Math.floor(stats.spd * 1.5);
							}
						}
		...
		if (allowTypeOverride) {
			...
			if (category !== 'Status' && !move.isZ && !move.id.startsWith('hiddenpower')) {
				if (moveType === 'Normal') {
					...
					if (value.abilityModify(0, 'Kablooey')) moveType = 'Electric';
	...
	getPokemonTypes(pokemon: Pokemon | ServerPokemon, preterastallized = false): ReadonlyArray<TypeName> {
		if (pokemon.speciesForme === "Giratina-Primal" && preterastallized) {
