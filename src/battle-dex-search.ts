...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'letsgo' | 'metronome' | 'natdex' | 'insurgencenatdex' | 'nfe' |
	...
	getTier(pokemon: Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'insurgencenatdex' ? `gen9insurgencenationaldex` :
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		if ((format.endsWith('cap') || format.endsWith('caplc')) && dex.gen < 9) {
			...
			table = table['gen9insurgencenationaldex'];
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			...
			(dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'insurgencenatdex');
		...
		if (this.formatType?.startsWith('insurgence')) lsetTable = lsetTable['gen9insurgencenationaldex'];
