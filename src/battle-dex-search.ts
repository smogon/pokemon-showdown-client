...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	private moveIsNotUseless(id: ID, species: Species, moves: string[], set: PokemonSet | null) {
		...
		case 'burnup':
			return dex.gen >= 9 && set?.teraType === 'Fire';
		case 'chillingwater':
		...
		case 'darkvoid':
			return dex.gen < 7;
		case 'doubleshock':
			return dex.gen >= 9 && set?.teraType === 'Electric';
		case 'dualwingbeat':
		...
		case 'hiddenpowerfire':
			return (dex.gen < 4 && !moves.includes('firepunch')) && !moves.includes('flamethrower') && !moves.includes('burningjealousy');
