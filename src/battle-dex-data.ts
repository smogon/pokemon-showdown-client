	...
	constructor(id: ID, name: string, data: any) {
		...
		if (!this.gen) {
			if ([406, 407, 424, 429, 430, 433, 438, 439, 440, 446, 458, 461, 462, 463, 464, 465, 466, 467,
				...
				990, 991, 992, 995, 1005, 1006].includes(this.num) || this.formeid === '-alola' ||
				(this.formeid.startsWith('-galar') || this.formeid.startsWith('-paldea') ||
					this.formeid.startsWith('-hisui')) && [52, 77, 78, 83, 110, 122, 222, 263, 264, 79, 80,
					144, 145, 146, 199, 58, 59, 100, 101, 157, 211, 215, 128, 194].includes(this.num)) {
				this.gen=3
			}
			else if (this.num >= 906 || this.formeid.startsWith('-paldea')) {
				this.gen = 9;
